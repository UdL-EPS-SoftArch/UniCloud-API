Feature: Register User
  ...
  Scenario: Register existing username
    Given There is a registered user with username "user" and password "existing" and email "user@mytournamentx.game"
    And I'm not logged in
  ...
  Scenario: Register user with an existing email
    Given There is a registered user with username "user" and password "password" and email "user@mytournamentx.game"
    And I'm not logged in
    When I register a new user with username "user2", email "user@mytournamentx.game" and password "password2"
    Then The response code is 409
    And I can login with username "user" and password "password"